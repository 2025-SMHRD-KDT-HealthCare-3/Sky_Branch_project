...
function Home({ goAnalyze, analysisResult, startCoaching }) {
  ...
  const getFatigueStyle = () => {
    ...
    const level = analysisResult.fatigue;
    if (level === 'high') return { bg: 'rgba(239,68,68,0.1)', border: 'rgba(239,68,68,0.3)', color: '#ef4444', icon: '🔴', label: '높음 — 즉각 조치 필요' };
    if (level === 'mid') return { bg: 'rgba(245,158,11,0.1)', border: 'rgba(245,158,11,0.3)', color: '#f59e0b', icon: '🟡', label: '주의 — 관리 필요' };
    return { bg: 'rgba(34,197,94,0.1)', border: 'rgba(34,197,94,0.3)', color: '#22c55e', icon: '🟢', label: '낮음 — 양호한 상태' };
  };
